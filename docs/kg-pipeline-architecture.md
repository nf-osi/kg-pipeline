...
- **BioLink Model for schema-level type assertions** — where a BioLink class exists (e.g., `biolink:CellLine`, `biolink:Gene`, `biolink:Study`), assert it as an `rdf:type`. Whether it is the *only* type turns on one question: **does a portal table back this entity?** If yes, keep an `nf:` class as an `rdfs:subClassOf` the BioLink parent and emit both types — the entity's portal-derived slots need an `rdfs:domain` we define, and the index does no OWL reasoning, so `rdfs:subClassOf` alone leaves either side of the pair matching nothing. If no (text-mined or externally sourced entities, e.g. `biolink:ChemicalEntity` from PubTator3), use the BioLink class directly with no `nf:` class. See `docs/biolink-alignment.md`. This provides a graph-native vocabulary shared with federated KG projects and enables cross-portal querying without requiring full ontology alignment.
...
For the NF portal, BioLink alignment is implemented as described [docs/biolink-alignment.md](biolink-alignment.md).
