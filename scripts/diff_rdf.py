...
def load_rdf_dir(rdf_dir: Path) -> Store:
    """Load all non-raw TTL files from a directory into a single store."""
    store = Store()
    ...
    logger.info("Loaded %d triples from %d files", len(store), len(ttl_files))
    return store
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=f"Directory containing current TTL files (default: {DEFAULT_RDF_DIR})",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logger.info("Loading current RDF from %s", args.rdf_dir)
        curr_store = load_rdf_dir(args.rdf_dir)
